...
export function FactorLab() {
  ...
  const start = async () => { try { let selected=members.split(/[\s,]+/).filter(Boolean); if(poolType!=="CUSTOM_STATIC_POOL") selected=(await getIndexSnapshot(release,indexCode,snapshotDate)).members; const r = await createFactorBatch({ release_id: release, members:selected, start_date: "2020-01-01", end_date: "2024-12-31", alpha_ids: ready.map(x => x.alpha_id), pool_type: poolType, snapshot_date: poolType==="CUSTOM_STATIC_POOL"?null:snapshotDate }); setBatch(r.experiment_id); } catch (e) { setError(String(e)); } };
