...
import { createETFExperiment, getETFExperiment, getETFTemplates, getRunReportUrl, listDataHubReleases, preflightETF, type ETFGroup, type ETFTemplate, type ReleaseSummary } from "../api";
...
export function ETFResearch() {
  ...
  const [result, setResult] = useState<Array<{ template: string; blocked: boolean; missing: Array<Record<string, string>> }>>([]); const [group, setGroup] = useState<ETFGroup | null>(null); const [error, setError] = useState(""); const [loading, setLoading] = useState(false);
