...
type Row = { key: string; name: string; published?: { stocks?: number; first_date?: string; latest_date?: string; qualification?: string }; usage: string; qualification: string };
...
export function DataCoverageTable({ rows }: { rows: Row[] }) {
  return <Card title="数据完整度"><Table rowKey="key" pagination={false} size="middle" scroll={{ x: 720 }} dataSource={rows} columns={[
    ...
    { title: '可用于研究', render: (_, row) => <Tooltip title={row.qualification}>{qualification(row.qualification)}</Tooltip> },
    { title: '股票覆盖', render: (_, row) => fmt(row.published?.stocks) },
    { title: '时间范围', render: (_, row) => row.published?.first_date ? `${row.published.first_date} — ${row.published.latest_date ?? '未提供'}` : '未提供' },
