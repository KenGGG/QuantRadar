...
export function DataCollectorProcessCard({ collector }: { collector: CollectorProcess }) {
  ...
  const details = collector.activities.map(activity => ({ key: activity.activity_id, queue: activity.queue ?? '未提供', purpose: queueMeta[activity.queue ?? '']?.title ?? activity.queue ?? '未提供', running: activity.task_progress.running, pending: activity.task_progress.pending, completed: activity.task_progress.completed, total: activity.task_progress.total }));
  const total = collector.activities.reduce((sum, item) => sum + (item.task_progress.total ?? 0), 0);
  const completed = collector.activities.reduce((sum, item) => sum + (item.task_progress.completed ?? 0), 0);
  return <Card title={<Space><Typography.Text strong>{title}</Typography.Text><Tag color={collector.status === 'RUNNING' || collector.status === 'PUBLISHING' ? 'processing' : undefined}>{statusText}</Tag></Space>}>
    <Space direction="vertical" size={10} style={{ width: '100%' }}>
      ...
      <Descriptions size="small" column={{ xs: 1, sm: 2 }}><Descriptions.Item label="当前工作">{work.length ? work.join(' + ') : collector.status === 'PENDING' ? '等待重试' : '批量处理中'}</Descriptions.Item><Descriptions.Item label="当前证券">{currentItems[0] ?? '未在请求'}</Descriptions.Item><Descriptions.Item label="当前批次">{fmt(running.reduce((sum, item) => sum + (item.task_progress.running ?? 0), 0))}项</Descriptions.Item><Descriptions.Item label="任务进度">{fmt(completed)} / {fmt(total)}</Descriptions.Item><Descriptions.Item label="后台待处理">{fmt(collector.pending)}</Descriptions.Item><Descriptions.Item label="最后心跳">{time(collector.heartbeat)}</Descriptions.Item></Descriptions>
      ...
      <Collapse items={[{ key: 'queues', label: '查看任务详情', children: <Table rowKey="key" pagination={false} size="small" dataSource={details} columns={[{ title: '队列', dataIndex: 'queue' }, { title: '用途', dataIndex: 'purpose' }, { title: '正在处理', render: (_, row) => fmt(row.running) }, { title: '待处理', render: (_, row) => fmt(row.pending) }, { title: '已完成', render: (_, row) => fmt(row.completed) }, { title: '总计', render: (_, row) => fmt(row.total) }]} /> }]} />
    </Space>
  </Card>;
}
