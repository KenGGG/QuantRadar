...
export function groupCollectorProcesses(activities: DataHubActivity[]): CollectorProcess[] {
  ...
  return [...groups.entries()].map(([key, members]) => ({
    ...
    activities: members, status: members.some(item => item.status === 'RUNNING') ? 'RUNNING' : members.some(item => item.status === 'PENDING') ? 'PENDING' : 'COMPLETED',
...
export function DataCollectorProcessCard({ collector }: { collector: CollectorProcess }) {
  ...
  const title = `${collector.source ?? '未提供'} ${collector.dataset === 'trade_status' ? 'ST / 停牌采集' : `${collector.dataset} 采集`}`;
  ...
  return <Card title={<Space><Typography.Text strong>{title}</Typography.Text><Tag color={collector.status === 'RUNNING' ? 'processing' : undefined}>{collector.status === 'RUNNING' ? '运行中' : collector.status === 'PENDING' ? '待处理' : '已完成'}</Tag></Space>}>
    <Space direction="vertical" size={10} style={{ width: '100%' }}>
      ...
      <Typography.Text>正在下载：{collector.dataset === 'trade_status' ? 'ST / 停牌' : collector.dataset}</Typography.Text>
