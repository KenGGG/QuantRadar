...
export const ReturnOverview = memo(function ReturnOverview({ data, compact = false }: { data: NativeReportData | null; compact?: boolean }) {
  const container = useRef<HTMLDivElement>(null);
  const returnsChart = useRef<ReactECharts>(null);
  const assetsChart = useRef<ReactECharts>(null);
  useEffect(() => {
  ...
  return <div ref={container} className={`return-overview ${compact ? "compact" : ""}`}>
    ...
    <div className="returns-chart">{data && rows.length ? <ReactECharts ref={returnsChart} autoResize={false} option={option} style={{ height: "100%", minHeight: compact ? 260 : 310 }} notMerge /> : <div className="chart-empty"><span>收益曲线</span><p>设置回测区间和资金，点击「运行策略回测」查看结果</p></div>}</div>
    {!compact && data && <div className="asset-chart"><div className="panel-heading">每日资产与持仓市值</div><ReactECharts ref={assetsChart} autoResize={false} style={{ height: 210 }} option={{ animation: false, tooltip: { trigger: "axis" }, legend: { top: 8 }, grid: { left: 80, right: 28, top: 40, bottom: 28 }, xAxis: { type: "category", data: rows.map(r => r.date.slice(0, 10)), axisLabel: { color: "#7d8b9e" } }, yAxis: { type: "value", scale: true, splitLine: { lineStyle: { color: "#e4e8ec" } } }, series: [{ name: "总资产", field: "total_value", color: "#4179b5" }, { name: "持仓市值", field: "positions_value", color: "#28a3b7" }, { name: "可用资金", field: "cash", color: "#a3b371" }].map(s => ({ name: s.name, type: "line", showSymbol: false, itemStyle: { color: s.color }, data: rows.map(r => r[s.field] === "" || r[s.field] == null ? null : Number(r[s.field])) })) }} /></div>}
  </div>;
});
