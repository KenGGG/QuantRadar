...
export function DataStatus() {
  ...
  return <Space direction="vertical" size={16} style={{ width: '100%' }}>
    ...
    <Collapse items={[{ key: 'advanced', label: '高级操作与版本详情', children: <Space direction="vertical" style={{ width: '100%' }}>
      <Space wrap><Button href={gapLedgerUrl} disabled={!gapLedgerUrl} target="_blank">打开固定版本字段缺口台账</Button><Button disabled={busy || !!running} onClick={() => action(() => updateAllData('audit'))}>检查候选数据</Button><Button disabled={busy || !!running || !data?.job.counts.failed} onClick={() => action(() => dataHubJobAction('repair'))}>重试失败项</Button><Button disabled={busy || !!running || (data?.job.counts.failed ?? 0) < 3} onClick={() => action(() => dataHubJobAction('health_probe'))}>抽样检查失败来源</Button><Button disabled={!data?.job.worker_alive || busy} onClick={() => action(() => dataHubJobAction('pause'))}>暂停采集</Button><DatePicker.RangePicker onChange={(_, values) => setRange(values[0] && values[1] ? values as [string, string] : null)} /><Button disabled={!range || busy || !!running} onClick={() => action(() => updateAllData('backfill', range?.[0], range?.[1]))}>历史回填</Button></Space>
