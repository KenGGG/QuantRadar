...
export interface FactorBatchSummary { status: string; error?: string; requested: number; completed: number; researcher_state: "NOT_READY" | "AWAITING_RESEARCHER_SELECTION" | "FROZEN_AWAITING_HOLDOUT_EVALUATION" | "HOLDOUT_ACCESSED"; representative_selection?: { alpha_ids: number[]; reasons: Record<string,string>; frozen_at: string } | null; holdout_access?: Record<string, unknown> | null; items: Array<{ alpha_id: number; calculation: string; evaluations: Record<string, { status: string; summary: { valid_dates: number; ic_mean: number | null; rank_ic_mean: number | null } }> }>; }
