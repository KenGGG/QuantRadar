...
def test_pipeline_rerun_skips_successful_prepare_and_analyze_stages(tmp_path) -> None:
    ...
    report = _setup_report(store, tmp_path, "A" * 32)
    calls = {"collect": 0, "prepare": 0, "analyze": 0}

    class Collector:
        def __init__(self, *_args): pass
        def collect(self, _target_date): calls["collect"] += 1; return {}

    def prepare(_store, _settings, item):
        calls["prepare"] += 1
        path = tmp_path / f"{item.id}.md"; path.write_text("正文", encoding="utf-8")
        return _store.save_markdown_artifact(item.id, path, "a" * 64, parser="mineru", parser_version="3.4.4", parse_quality={"status": "PARSE_OK"})
    ...
    assert first.prepared == first.analyzed == 1
    assert second.prepared == second.analyzed == 0
    assert calls == {"collect": 2, "prepare": 1, "analyze": 1}
    assert report.id > 0


def test_pipeline_continues_after_one_report_prepare_failure(tmp_path) -> None:
    from quantradar.research.config import ResearchSettings
    from quantradar.research.pipeline import run_pipeline
    from quantradar.research.storage import ResearchStore
    ...
    settings = ResearchSettings(f"sqlite+pysqlite:///{tmp_path / 'db.sqlite'}", tmp_path / "data", tmp_path / "profile")
    store = ResearchStore(settings); store.create_schema()
    first = _setup_report(store, tmp_path, "A" * 32)
    second = _setup_report(store, tmp_path, "B" * 32)
    analyzed = []

    class Collector:
        def __init__(self, *_args): pass
        def collect(self, _target_date): return {}

    def prepare(_store, _settings, item):
        if item.id == first.id:
            raise RuntimeError("MinerU temporary error")
        path = tmp_path / f"{item.id}.md"; path.write_text("正文", encoding="utf-8")
        return _store.save_markdown_artifact(item.id, path, "b" * 64, parser="mineru", parser_version="3.4.4", parse_quality={"status": "PARSE_OK"})

    def analyze(_store, report_id, _markdown, *_args):
        analyzed.append(report_id)

    result = run_pipeline(settings, date(2026, 8, 24), store=store, collector_cls=Collector, prepare_fn=prepare, analyze_fn=analyze)

    assert result.prepare_failed == 1
    assert result.analyzed == 1
    assert analyzed == [second.id]
