...
```text
当前阶段：Phase 3（JoinQuant 兼容层）— 已完成
阶段标志：JQ_COMPAT_CORE_PASS
最近完成：JoinQuant 兼容核心（get_price/history/attribute_history 经 Generic Provider Registry 接入；frequency 别名 d/day/1d->daily；字段别名 money->amount；fq='none' 原始价 + fq='pre'/'post' 等价原始价 LIMIT 透传，绝不伪造复权；与原表抽样对账一致，close≠adjclose 已验证）
...
InvestmentDataProvider       BASE IMPLEMENTED（Phase 2A）+ get_price PASS（Phase 2B）+ JQ 兼容核心 PASS（Phase 3）；get_split_dividend NOT IMPLEMENTED（Phase 5）
...
Phase 4（下一阶段）：真实 A 股回测（用 InvestmentDataProvider 跑通 BulletTrade BacktestEngine，验证端到端数据链路与防未来数据机制）
