...
- 当前活动目标为 `DATA_V4_G5_OFFLINE_MATRIX_REPLAY`。G4 已验证 FactorLab 与回测共用固定 release Provider、发布版生命周期 universe 和交易日预热；#1/#56 可算，#58/#48/#59 因未验证的历史申万字典明确阻塞。旧的 ETF／FactorLab P3 状态仅为冻结历史里程碑，不是当前目标。
...
- G2 market status worker has completed 210 historical and 44 current-window tasks, satisfied 34 historical and 5 current tasks without repeat download, and quarantined 53 confirmed historical BaoStock source limitations; 5,252 historical checks and 5,507 current-window checks remain pending. The fixed-release 20-trading-day audit found 17,764 valid of 208,552 lifecycle-qualified status fields; the exact evidence is in [G2 progress](acceptance/datahub-v4/g2-market-status-progress.md).
