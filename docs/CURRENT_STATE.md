...
- Current release is `Rd7dce3af4ec0da28`, pairing base commit `uhdpedb4pr97ve80aq6nrabr66atsqtq` with supplemental commit `018ncgvao7k4ga6s722nltici5jp8git`. The valuation failure baseline remains 196 parsing failures plus 123 unverified-coverage records; no failed security was silently reclassified or deleted.
...
- G2 market status worker has completed 15 tasks, satisfied 7 already-published tasks without repeat download, and quarantined 9 confirmed BaoStock source limitations; 5,518 release-audited checks remain pending. The exact evidence is in [G2 progress](acceptance/datahub-v4/g2-market-status-progress.md).
