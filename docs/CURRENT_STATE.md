...
- 当前活动目标为 `WEBUI_P3_FACTOR_SELECTION_AND_INDUSTRY_ETF`；P0 不可变实验／release 门禁、P1 ETF 研究组和 P2 FactorLab 已独立可用。P3-A 为 `AWAITING_RESEARCHER_SELECTION`：研究者须在页面填写理由并冻结代表因子，系统不会按收益自动选择。P3-B 为 `BLOCKED_DATA_IDENTITY`：五行业 ETF 缺少本地可验证的跟踪指数身份，但不影响已开放的 ETF_RAW 研究工作台。
