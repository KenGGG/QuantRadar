...
- Current release is `R05b433fd8803f501`, pairing base commit `0o6tgnmq5vabt3orqrk8rhqptniae26l` with supplemental commit `66fg9p699hveeka64udf0h5u4bimcsdu`. The valuation failure baseline remains 196 parsing failures plus 123 unverified-coverage records; no failed security was silently reclassified or deleted.
...
- G2 market status worker has completed 159 tasks, satisfied 39 already-published tasks without repeat download, and quarantined 34 confirmed BaoStock source limitations; 5,322 historical checks and 5,551 current-window checks remain pending. The fixed-release 20-trading-day audit found 17,764 valid of 208,552 lifecycle-qualified status fields; the exact evidence is in [G2 progress](acceptance/datahub-v4/g2-market-status-progress.md).
- The complete fixed-release G2 audit over 1990-12-19 through 2026-09-14 found 29,812,010 valid of 36,959,510 lifecycle-qualified `tradestatus`/`is_st` fields (80.6612695893425%), with 7,147,500 missing fields compressed into 32,690 intervals. It includes all 5,556 supported SH/SZ securities, zero lifecycle-unknown denominators, and 343 explicitly unsupported BSE securities.
...
- The user-level `quantradar-datahub.timer` is installed and active. It triggers the dedicated G2-only DataHub coordinator at 20:00 Asia/Shanghai; the coordinator handles the current 20-trading-day status correction window and 50 persisted historical status tasks per run, while explicitly skipping valuation and industry collection.
- The daily target is capped at the latest observed base-price trading date as well as the exchange calendar, so a wall-clock date cannot create a false current status gap.
- The dedicated status maintenance service completed an end-to-end user-timer catch-up on 2026-09-15, verified base commit `0o6tgnmq5vabt3orqrk8rhqptniae26l`, refreshed the 5,899-security master, and published `R91ec6235708e3d10` without starting legacy valuation collection. A follow-up fixed the paired-release alignment and published `Rb7a2cd281c8e2ebe` after status maintenance; its current-window checks all satisfied without a repeated source request.
