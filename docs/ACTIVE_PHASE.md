...
- P2：BaoStock 单次登录和一会话三只股票的最小样本成功，原始响应可同时派生行情、状态与估值候选；3 条真实缺失状态记录已在独立部分 release 发布并通过回放。剩余尾部仍为 UNKNOWN。
- P3：旧、新 release 的原低 Beta 2023-09 回放均通过并产生相同哈希；状态补丁未触及该策略的实际状态依赖路径，差异归因为零。SQL 查询性能仍需按批量读取与缓存继续优化。
