...
- **G1 Index Snapshot — IN_PROGRESS.** 使用 `qr_index_snapshot_version` 和引用其不可变 `snapshot_id` 的成员/权重表保存 CSI 每交易日和申万一级每周快照。业务 `content_hash` 不含抓取元数据或行顺序；同内容为 `NO_CHANGE`，变更追加 revision。20:30 Asia/Shanghai 必须读取固定交易日历；不伪造非交易日快照。
