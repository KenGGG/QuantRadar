...
def validate_analysis(analysis: dict[str, Any], chunks: list[SourceChunk], *, report_id: int | None = None, markdown_sha256: str | None = None) -> ValidationResult:
    ...
    errors = [f"ANALYSIS_MISSING_FIELD:{field}" for field in (
        "research_type", "one_line_summary", "key_points", "core_conclusion",
        "method_or_logic", "risks_or_limitations",
    ...
    for item in analysis.get("evidence", []):
        if not isinstance(item, dict):
            errors.append("EVIDENCE_INVALID_ITEM")
            continue
